...
text_label <- function( dt, match ) {
  dt <- dt[ source==match ]
  fit <- lm(pollutants ~ year, data=dt)
...
graph <- function(dt) {
  ...
  #p <- p + geom_text(x=2005,y=2100, label=text_label(dt,"POINT"), size=4, family="serif", fontface="plain", color=factor("POINT") )
  #p <- p + geom_text(x=2005,y=2000, label=text_label(dt,"NONPOINT"), size=4, family="serif", fontface="plain", color="green" )
