...
balt_cars <- mergedt[ fips=="24510" & Short.Name %like% "Highway", sum(Emissions), by=year ]
setnames(balt_cars, "V1", "pollutants")
balt_cars$area <- "Baltimore City"
balt_cars <- balt_cars[ order( balt_cars$year ), ]

la_cars <- mergedt[ fips=="06037" & Short.Name %like% "Highway", sum(Emissions), by=year ]
...
graph <- function(data_dt) {
  ...
  legend_text <- paste( c("Change =", slope, "Tons/year" ),  collapse = " ")
